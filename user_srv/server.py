...
from user_srv.config import server_config
...
def on_exit(signal, frame):
    ...
    if not c.deregister(service_id=server_config.SERVER_NAME):
        logger.error("服务注销失败")
...
def serve():
    ...
    logger.info("服务注册中...")
    c = ConsulRegister(host=server_config.CONSUL_HOST, port=server_config.CONSUL_PORT)
    success = c.register(name=server_config.SERVER_NAME, service_id=server_config.SERVER_NAME, address=server_config.HOST,
               port=server_config.PORT, tags=server_config.SERVER_TAGS, check=None)
    print(success)
    if not success:
        logger.error("服务注册失败")
        sys.exit(0)
